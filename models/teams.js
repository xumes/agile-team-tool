...
module.exports.validateMemberUserData = /* istanbul ignore next */ function(team, userIds) {
  return new Promise(function(resolve, reject){
    var promiseArray = [];
    var errorData = [];
    Users.getUsersInfo(_.uniq(userIds))
      .then(function(users) {
        _.each(users, function(user, index) {
          var memberData = _.find(team.members, function(m) {
            if (m.userId == user.userId)
              return m;
          });
          if (!_.isEmpty(memberData)) {
            if (_.isEmpty(memberData.email) || _.isEmpty(user.email) ||
                memberData.userId.toUpperCase() != user.userId.toUpperCase() ||
              ...
              var user = {
                ...
                email: user.email || '',
                userId: user.userId || '',
                site: user.location ? user.location.site : '',
                timezone: user.location ? user.location.timezone : ''
              };
              var data = {
                teamId : team._id,
                teamName: team.name,
...
module.exports.validateMemberData =  /* istanbul ignore next */ function(teamId) {
  return new Promise(function(resolve, reject){
    var promiseArray = [];
    var errorData = [];
    var teams = [];
