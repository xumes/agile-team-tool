...
function teamAssessmentListHander(teamId, assessments) {
	assessmentData = sortAssessments(assessments);
	loadAssessmentListTable(teamId, false);
}

function loadAssessmentListTable(teamId, more){
	$("#assessmentList").empty();
	var numRec = 5;
	if (more){
		numRec= assessmentData.length;
	}
	var link='';
	if (assessmentData != null && assessmentData.length > 0){
		...
		if (assessmentData.length > 5 && numRec == 5){
			$("#assessmentTitle").html("Last 5 Assessments for " + $("#teamSelectList option:selected").text());
			$("#moreLink").text($("#moreLink").text().replace('Less', 'More'));
			$("#moreLink").attr('href','javascript:showMore()');
			$("#moreAssessments").show();
		}
		else if(assessmentData.length > 5 && numRec > 5){
			$("#assessmentTitle").html("All Assessments for " + $("#teamSelectList option:selected").text());
			$("#moreLink").text($("#moreLink").text().replace('More', 'Less'));
			$("#moreLink").attr('href','javascript:showLess()');
			$("#moreAssessments").show();
		} else {
			$("#assessmentTitle").html("Last 5 Assessments for " + $("#teamSelectList option:selected").text());
...
function showMore(){
	loadAssessmentListTable($("#teamSelectList option:selected").val(), true);
}

function showLess(){
	loadAssessmentListTable($("#teamSelectList option:selected").val(), false);
}
...
function retrieveAssessmentResult(teamId, assessId){
	getAllAgileTeams(teamListHandler, [teamId, assessId]);
}

function teamListHandler(teamId, assessId, teamList){
	setGlobalTeamList(teamList);
	var teamLookup = getLookupListById(teamList);
