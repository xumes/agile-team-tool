...
var TypeAhead = function (inputBox, settings) {
    ...
    var highlightNext = function() {
        if ( this.isPopupShowing() ) {
            ...
            //Highlight first item
                ...
                var $jul = h.query(this.container, 'ul:not(ul:empty)');
                for ( var i=0; i<$jul.length; i++ ) { 
                    //If visible, highlight its first child
                    if ( $jul[i].style.display != 'none' ) {
                        _highlight(null, h.query(h.queryAll($jul[i]), 'li:first-child'));
                        break;
                    }
                }
            }
     ...
     */
    function _highlight($jcurr, $jfuture) {
        if ( $jcurr ) { 
            $jcurr.removeClass(selectedClass); 
        }
        if ( $jfuture && $jfuture.length > 0 ) {
            $jfuture.addClass(selectedClass);
            h.setAttr(inputBox, 'aria-activedescendant', $jfuture[0].id);
        } else {
    ...
    this.hideResults = function(feature) {
       ...
       if ( hideAll ) {
           if (window.jQuery) {
               $j('body').unbind('.faces-typeahead-hide');
           } else if (window.dojo) {
