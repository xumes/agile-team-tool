...
```
{
  ...
  active: false,
  display: '[Replace with a system wide message to display if `sysStatus.currentFlag` is set to `DynamicChange`]',
  adminAccess: 'none'
},
{
  ...
  active: false,
  display: '[Replace with a system wide message to display if `sysStatus.currentFlag` is set to `AdminOnlyChange`]',
  adminAccess: 'full'
}
```
